  ...
  function startPractice() {
    ...
    function next() {
      ...
      if (remaining.length === 0) {
        document.querySelector(".module-section").innerHTML =
          "<h2>✓ Fruits Mastered</h2>";
        localStorage.setItem("fruits_mastered", "true");
        return;
      }

      current = remaining[Math.floor(Math.random() * remaining.length)];

      prompt.textContent = "Tap this fruit:";
    ...
    options.addEventListener("click", function(e){
      ...
      if (target.dataset.value === current) {
        ...
        remaining = remaining.filter(f => f !== current);
        setTimeout(() => {
